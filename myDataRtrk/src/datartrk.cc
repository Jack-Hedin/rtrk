...
datartrk::datartrk(const std::string& recojetnameR04,
  ...
  //, trigAna(nullptr)
{
  cout << "datartrk running in DATA mode" << endl;
}
//____________________________________________________________________________..
datartrk::~datartrk()
{
  cout << "datartrk::Destructor - Cleaning up" << endl;
  //delete trigAna;
  //trigAna = nullptr;
}
//____________________________________________________________________________..
...
//____________________________________________________________________________..
float datartrk::CalculateDeltaR(float eta1, float phi1,
...
//____________________________________________________________________________..
bool datartrk::processVertex(PHCompositeNode *topNode)
{
...
//____________________________________________________________________________..
bool datartrk::processTrigger(PHCompositeNode *topNode)
{
  bool trig22 = false, trig23 = false;
  ...
  try
  {
    ...
    if (gl1PacketInfo)
    {
      ...
      for (int i = 0; i < 64; i++)
      {
        ...
        if (trig_decision && i == 22) trig22 = true;
        if (trig_decision && i == 23) trig23 = true;
        triggervec = (triggervec >> 1U) & 0xffffffffU;
      }
    }
    else
    {
      if (m_total_events <= 10)
      {
        cout << "WARNING: GL1Packet not found for event " << m_total_events
             << ", continuing with z-vertex cut only" << endl;
      }
      trig22 = trig23 = true;
    }
  ...
  catch (...)
  {
    if (m_total_events <= 10)
    {
      cout << "WARNING: Trigger selection failed for event " << m_total_events
           << ", continuing with z-vertex cut only" << endl;
    }
    trig22 = trig23 = true;
  }

  return (trig22 || trig23);
}

//____________________________________________________________________________..
int datartrk::processRtrkAnalysis(PHCompositeNode *topNode)
{
